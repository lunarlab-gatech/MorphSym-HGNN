...
class COM_HGNN(torch.nn.Module):
    """
    The COM Heterogeneous Graph Neural Network model.
    """
    ...
    def __init__(self, hidden_channels: int, num_layers: int, data_metadata, 
        ...
        self.activation = activation_fn
        self.num_bases = 1
        self.num_dimensions_per_base = com_dimension
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for conv in self.convs:
            x_dict = conv(x_dict, edge_index_dict)
            x_dict = {key: self.activation(x) for key, x in x_dict.items()}
        return self.decoder(x_dict['base'])