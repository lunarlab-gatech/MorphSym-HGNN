...
def draw_vector(pb, origin, vector, v_color, scale=1.0):
    ...
    # import morpho_symm
    cone_path = pathlib.Path(__file__).parent / "resources/stl_files/Cone.obj"
...
def change_robot_appearance(pb, robot: PinBulletWrapper, change_color=True, alpha: float = 1.0, group_name="C2"):
    ...
    robot_color = [0.5,0.5,0.5, alpha]  # This is a nice teal
    if group_name == "Klein4":
        # solo color
        FL_leg_color = [0.9, 0.5, 0.2, alpha]   # Deeper, rich warm orange    
        FR_leg_color = [0.3, 0.6, 0.9, alpha]   # Lighter, soft teal
        HL_leg_color = [0.8, 0.7, 0.2, alpha]   # Deeper, golden yellow    
        HR_leg_color = [0.5, 0.7, 0.8, alpha]   # Lighter, vivid sky blue
    if group_name == "C2":
        ## a1 color
        FL_leg_color = [0.8, 0.7, 0.2, alpha]   # Deeper, rich warm orange    
        FR_leg_color = [0.5, 0.7, 0.8, alpha]   # Lighter, soft teal
        HL_leg_color = [0.8, 0.7, 0.2, alpha]   # Deeper, golden yellow    
        HR_leg_color = [0.5, 0.7, 0.8, alpha]   # Lighter, vivid sky blue
...
def display_robots_and_vectors(pb, robot, group, base_confs, orbit_q_js, orbit_v_js, orbit_com_momentum, forces,
    ...
    for g in G.elements:
        ...
        if g != G.identity:
            ...
            if tint:
                change_robot_appearance(pb, grobot, group_name=G.name)
