...
# import morpho_symm
from pybullet_visual_utils import (
...
from robot_utils import load_symmetric_system
...
def main(cfg: DictConfig):
    ...
    change_robot_appearance(pb, robot, change_color=cfg.robot.tint_bodies, alpha=1.0, group_name=G.name)  # Add color and style to boring grey robots
    ...
    # XB[:3, :3] = np.eye(3)  # Set the orientation of the robot base to identity matrix.
    ...
    if cfg.make_gif:
        ...
        single_view_matrix = pb.computeViewMatrixFromYawPitchRoll(
            cameraTargetPosition=XB[:3, 3] + np.array([0, 0, 0]),
            distance=0.8,
            upAxisIndex=2,
            roll=0,
            pitch=-90,
        ...
        im.save(save_path / "single_view.png")
        
        frontal_view_matrix = pb.computeViewMatrixFromYawPitchRoll(
            cameraTargetPosition=XB[:3, 3] + np.array([0, offset, 0]),
            distance=1.3,
            upAxisIndex=2,
            roll=0,
            pitch=-35,
        ...
        im.save(save_path / "back_view.png")
